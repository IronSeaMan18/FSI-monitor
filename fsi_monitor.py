#!/usr/bin/env python3
"""
FSI Vessel Arrival Monitor v3.8.0 — Hardened
=============================================
...
v3.8.0 fixes all 19 audited weak points. See PLAN-v3.8.0.md.
"""
...
def parse_eta(raw):
    ...
    try:
        ...
        # DD/MM [HH:MM]  (Marin)
            ...
            hh = int(m.group(3) or 0); mi = int(m.group(4) or 0)
            yr = now.year
...
def queue_flags(imos):
    with _resolve_lock:
        for i in imos:
            if i and not get_flag(i) and i not in _resolve_seen:
                _resolve_seen.add(i); _resolve_q.append(i)
...
const jobs=[];
for(const p of sp){
  if(p.direct)jobs.push({p,url:"/api/"+p.direct,tag:p.direct==="bilbao"?"PA":"PA"});
  ...
  if(p.vfsupp)jobs.push({p,url:"/api/port?vf="+p.vf+"&pid="+p.id+"&pname="+encodeURIComponent(p.name),tag:"VF+"});
}
...
el("tB").innerHTML=so.map(v=>{const imo=v.imo||"",nm=v.name||"",fc=v.flagCode||"",fn=v.flagName||"";const eta=v.eta||v.lastETA||"";const isLive=v._src==="live";const vk=vKey(v).replace(/'/g,"\\'");const checked=S.sel.has(vKey(v))?"checked":"";const lnk=imo?'<a href="https://www.vesselfinder.com/vessels/details/'+imo+'" target="_blank">🚢</a>':'<a href="https://www.vesselfinder.com/vessels?name='+encodeURIComponent(nm)+'" target="_blank">🔍</a>';return '<tr class="'+(isLive?"":"stale")+'"><td><input type="checkbox" class="ck" '+checked+' onchange="toggleSel(\''+vk+'\')"></td><td style="font-size:11px;font-family:var(--m);color:'+(isLive?"var(--amber)":"var(--t3)")+';white-space:nowrap">'+(v.etaISO?shortDate(v.etaISO):(eta||"—"))+'</td><td><div style="font-weight:600;color:var(--t1);font-size:12px">'+nm+"</div>"+(imo?'<div style="font-size:10px;color:var(--t3);font-family:var(--m)">IMO '+imo+"</div>":"")+'</td><td style="white-space:nowrap"><span style="font-size:14px">'+flagEmoji(fc)+'</span> <span style="font-size:11px;color:var(--t2)">'+fc+'</span><div style="font-size:9px;color:var(--t3)">'+fn+'</div></td><td style="font-size:11px;color:var(--t2)">'+(v.type||"—")+'</td><td style="font-size:11px;color:var(--t2);font-weight:600">'+(v.portName||"—")+'</td><td style="font-size:10px;color:var(--t3)">'+(v.origin||"—")+'</td><td style="font-size:11px;font-family:var(--m);color:var(--t2)">'+(v.gt?v.gt.toLocaleString():"—")+'</td><td style="font-size:11px;font-family:var(--m);color:var(--t2)">'+(v.dwt?v.dwt.toLocaleString():"—")+'</td><td>'+lnk+"</td></tr>"}).join("")}
