#!/usr/bin/env python3
"""
FSI Vessel Arrival Monitor v3.5.1 — Client-side flag resolution
==================================================
...
const S={ports:JSON.parse(localStorage.getItem("fsi10_p")||'["ESGIJ","ESAVS","ESTAN","ESBIO","ESPAS","FRBAY","ESSCI","ESFER","ESCOR","ESMRN","ESVIL","ESVGO"]'),flags:JSON.parse(localStorage.getItem("fsi10_f")||'["MT","LR","MH"]'),live:[],db:[],combined:[],sel:new Set(),q:"",ft:"all",src:"all",sb:"eta",sd:"asc",loading:false,logs:[],po:{port:false,flag:false},today:new Date().toISOString().slice(0,10)};
const sv=()=>{localStorage.setItem("fsi10_p",JSON.stringify(S.ports));localStorage.setItem("fsi10_f",JSON.stringify(S.flags))};
...
if __name__ == "__main__":
    ...
    print(f"""
╔═══════════════════════════════════════════════════════════╗
║  ⚓ FSI Vessel Arrival Monitor v3.5.1                          ║
