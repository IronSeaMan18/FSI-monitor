#!/usr/bin/env python3
"""
FSI Vessel Arrival Monitor v3.6.0 — Client-side flag resolution
==================================================
...
SN_PORTS = {
    ...
    # ESMRN (Marin) and ESVIL (Vilagarcia) not in ShipNext -> use VF
}
_imo_flag_cache = {}  # runtime IMO->flagCode cache

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DB_FILE = os.path.join(SCRIPT_DIR, "fsi_vessels_db.json")

UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
UA_BOT = "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"  # bypasses Radware/Cloudflare bot blocks
...
# ─── Flag resolution via VesselFinder ────────────────────
def resolve_flag(imo):
    """Resolve flag from cache or VF (using Googlebot UA to bypass bot blocks)."""
    if not imo: return ""
    if imo in _imo_flag_cache: return _imo_flag_cache[imo]
    # Check DB cache first
    db = load_db()
    fc = db.get("flags", {}).get(imo, "")
    if fc:
        _imo_flag_cache[imo] = fc
        return fc
    # Fetch from VesselFinder with Googlebot UA
    try:
        ...
        if "Radware" in html or "Verifying" in html:
            return ""
...
def save_flag(imo, fc):
    """Persist a browser-resolved flag to the DB cache."""
    if not imo or not fc: return
    fc = XMAP.get(fc.upper(), fc.upper())
    _imo_flag_cache[imo] = fc
    db = load_db()
    if "flags" not in db: db["flags"] = {}
    db["flags"][imo] = fc
    save_db(db)
...
def fetch_shipnext(port_id):
    ...
    if not sn_id:
        return []
    url = f"{SN_BASE}/{sn_id}/planned-vessels"
    ...
    for v in vessels:
        ...
        if eta_raw:
            try:
                ...
                d = dt.fromisoformat(eta_raw.replace("Z", "+00:00"))
                eta = d.strftime("%b %d, %H:%M")
            except:
                eta = eta_raw[:10]
        result.append({
            "name": name, "imo": imo,
    ...
    # Resolve flags: cache-first, then VF with Googlebot UA in parallel
    import concurrent.futures
    def _rf(imo): return imo, resolve_flag(imo)
    imos = [v["imo"] for v in result if v["imo"]]
    with concurrent.futures.ThreadPoolExecutor(max_workers=25) as ex:
        resolved = dict(ex.map(_rf, imos))
    for v in result:
        v["flagCode"] = resolved.get(v["imo"], "")
    return result
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path in ("/", "/index.html"):
            ...
            try:
                vs = fetch_shipnext(pid)
                db = load_db()
                a, u = merge_into_db(db, vs, pid, pname, "SN")
                save_db(db)
                self._json(200, {"vessels": vs, "count": len(vs), "added": a, "updated": u})
            except Exception as e:
                print(f"  ShipNext {pid}: {e}")
                self._json(500, {"error": str(e), "vessels": [], "count": 0})
        elif p.path == "/api/saveflag":
            params = parse_qs(p.query)
            imo = params.get("imo",[""])[0]
            fc = params.get("fc",[""])[0]
            if imo and fc:
                save_flag(imo, fc)
            self._json(200, {"imo": imo, "flagCode": fc, "saved": bool(imo and fc)})
...
const S={ports:JSON.parse(localStorage.getItem("fsi11_p")||'["ESGIJ","ESAVS","ESTAN","ESBIO","ESPAS","FRBAY","ESSCI","ESFER","ESCOR","ESMRN","ESVIL","ESVGO"]'),flags:JSON.parse(localStorage.getItem("fsi11_f")||'["MT","LR","MH"]'),live:[],db:[],combined:[],sel:new Set(),q:"",ft:"all",src:"all",sb:"eta",sd:"asc",loading:false,logs:[],po:{port:false,flag:false},today:new Date().toISOString().slice(0,10)};
const sv=()=>{localStorage.setItem("fsi11_p",JSON.stringify(S.ports));localStorage.setItem("fsi11_f",JSON.stringify(S.flags))};
...
async function resolveFlagClient(imo){
  // Try CORS proxies in order; extract flag from VesselFinder HTML
  const vfUrl="https://www.vesselfinder.com/vessels/details/"+imo;
  const XM={XB:"PT",XA:"DK",XI:"NO"};
  const proxies=[
  ...
  for(const mk of proxies){
    try{
      const r=await fetch(mk(vfUrl),{signal:AbortSignal.timeout(12000)});
      if(!r.ok)continue;
      const html=await r.text();
      const m=html.match(/flags\/4x3\/(\w+)\.svg/);
      if(m){let fc=m[1].toUpperCase();return XM[fc]||fc;}
    }catch(e){}
  }
  return "";
}
...
const vfSuppPorts=sp.filter(p=>p.vfsupp);
for(let i=0;i<vfSuppPorts.length;i++){const port=vfSuppPorts[i];lg("📡 VF-supp: "+port.name+"...");el("loadTxt").textContent=port.name+" (VF supplement)";try{const r=await fetch("/api/port?vf="+port.vf+"&pid="+port.id+"&pname="+encodeURIComponent(port.name));const d=await r.json();if(d.vessels){const existing=new Set(S.live.filter(v=>v.portId===port.id).map(v=>v.imo));let added=0;d.vessels.forEach(v=>{if(XMAP[v.flagCode])v.flagCode=XMAP[v.flagCode];if(v.imo&&!existing.has(v.imo)){v.portId=port.id;v.portName=port.name;S.live.push(v);added++}});lg("✓ "+port.name+": +"+added+" from VF")}}catch(e){lg("✗ VF-supp "+port.name+": "+e.message)}buildCombined();render()}
const vfOnlyPorts=sp.filter(p=>!p.sn&&!p.bcn&&!p.direct);
for(let i=0;i<snPorts.length;i++){const port=snPorts[i];lg("📡 SN: "+port.name+"...");el("loadTxt").textContent=port.name+" (ShipNext "+(i+1)+"/"+snPorts.length+")";try{const r=await fetch("/api/shipnext?pid="+port.id+"&pname="+encodeURIComponent(port.name));const d=await r.json();if(d.vessels&&d.vessels.length){d.vessels.forEach(v=>{v.portId=port.id;v.portName=port.name});S.live.push(...d.vessels);tA+=d.added||0;lg("✓ "+port.name+": "+d.count+" (SN, +"+(d.added||0)+" new)")}else lg("✗ SN "+port.name+": "+(d.error||"0 vessels"))}catch(e){lg("✗ SN "+port.name+": "+e.message)}buildCombined();render()}
for(let i=0;i<vfOnlyPorts.length;i++){const port=vfOnlyPorts[i];lg("📡 VF: "+port.name+"...");el("loadTxt").textContent=port.name+" (VF "+(i+1)+"/"+vfOnlyPorts.length+")";try{const r=await fetch("/api/port?vf="+port.vf+"&pid="+port.id+"&pname="+encodeURIComponent(port.name));const d=await r.json();if(d.vessels){d.vessels.forEach(v=>{if(XMAP[v.flagCode])v.flagCode=XMAP[v.flagCode];v.portId=port.id;v.portName=port.name});S.live.push(...d.vessels);tA+=d.added||0;lg("✓ "+port.name+": "+d.count+" (VF, +"+(d.added||0)+" new)")}else lg("✗ VF "+port.name+": "+(d.error||"0"))}catch(e){lg("✗ VF "+port.name+": "+e.message)}buildCombined();render()}

// CLIENT-SIDE flag resolution (browser reaches VesselFinder; Render is blocked)
const needFlag=S.live.filter(v=>v.imo&&!v.flagCode);
if(needFlag.length){lg("🏴 Resolving "+needFlag.length+" flags via your browser...");el("loadTxt").textContent="Resolving flags ("+needFlag.length+")...";
let done=0;
for(const v of needFlag){
  const fc=await resolveFlagClient(v.imo);
  if(fc){S.live.filter(x=>x.imo===v.imo).forEach(x=>x.flagCode=fc);
    fetch("/api/saveflag?imo="+v.imo+"&fc="+fc).catch(()=>{});}
  done++;
  if(done%5===0){el("loadTxt").textContent="Flags "+done+"/"+needFlag.length;buildCombined();render();}
}
lg("✓ Flags resolved ("+done+")");buildCombined();render();}
...
el("sP").innerHTML=S.ports.map(pid=>{const p=PORTS.find(x=>x.id===pid);if(!p)return"";const c=f.filter(v=>v.portId===pid).length;return '<button class="chip active" onclick="tP(\''+pid+'\')">'+(p.bcn||p.direct?'🌟 ':'')+p.name+' <span class="badge">'+c+'</span></button>'}).join("")||"<em style='font-size:11px;color:var(--t3)'>Select ports</em>";el("pC").textContent=S.ports.length;
  ...
  const fc_counts={MT:0,LR:0,MH:0};S.combined.forEach(v=>{if(fc_counts[v.flagCode]!==undefined)fc_counts[v.flagCode]++});
  const FL3=[{c:"MT",n:"Malta",e:"🇲🇹"},{c:"LR",n:"Liberia",e:"🇱🇷"},{c:"MH",n:"Marshall Islands",e:"🇲🇭"}];
  if(el("sF"))el("sF").innerHTML=FL3.map(f=>'<button class="chip '+(S.flags.includes(f.c)?"active":"")+'" onclick="tF(\''+f.c+'\')"><span style="font-size:13px">'+f.e+'</span> '+f.n+' <span class="badge">'+fc_counts[f.c]+'</span></button>').join("");
...
if __name__ == "__main__":
    ...
    print(f"""
╔═══════════════════════════════════════════════════════════╗
║  ⚓ FSI Vessel Arrival Monitor v3.6.0                          ║
