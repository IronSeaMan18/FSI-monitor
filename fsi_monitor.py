...
function miCard(g,i){
 ...
 if(m.ism){
  ...
  head='<h3 style="color:var(--t1);font-size:13px;margin:0 0 4px 0">🏢 '+esc(m.ism)+" — "+vs.length+' vessel(s)</h3><div style="font-size:10px;color:var(--t3);margin-bottom:4px">'+vb+(m.owner?' · owner: '+esc(m.owner):"")+(m.phone?' · ☎ '+esc(m.phone):"")+'</div><div style="font-size:10px;color:var(--t3);margin-bottom:4px">To: '+(m.ismEmail?'<span style="color:var(--blue);font-family:var(--m)">'+esc(m.ismEmail)+"</span>":'<span style="color:var(--red)">no email on file — phone or contact form</span>')+"</div>"+(m.notes?'<div style="font-size:10px;color:var(--t2);margin-bottom:6px;padding:6px 8px;background:#0c1520;border:1px solid var(--brd);border-radius:4px;line-height:1.5">'+esc(m.notes)+"</div>":"");
 }else{
